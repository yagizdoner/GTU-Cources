...
int isStuck(TILES maze[100][100], Coord currentTile){
	...
	else if(maze[currentTile.x][currentTile.y].type == EMPTY){
		
		maze[currentTile.x][currentTile.y].type == USED;
		
		currentTile.x -= 1; /* (x-1,y)*/
		isStuck(maze,currentTile);
		
		currentTile.x += 2 ; /* (x+1,y) */
		isStuck(maze,currentTile);
		
		currentTile.x -= 1 ;  /* (x,y-1) */
		currentTile.y -= 1 ;
		isStuck(maze,currentTile);
		
		currentTile.y += 2 ; /* (x,y+1) */
		isStuck(maze,currentTile);
	}
	return result;
}
...
Path shortestExit(TILES maze[100][100], Coord currentTile, Path path, int *minWeight){
	...
	if(maze[currentTile.x][currentTile.y].type == BORDER){
		...
		currentTile.x -= 1; /* (x-1,y)*/
		shortestExit(maze,currentTile,path,minWeight);
...
Path comfortableExit(TILES maze[100][100], Coord currentTile, Path path, int *minWeight){
	...
	if(maze[currentTile.x][currentTile.y].type == BORDER){
		...
		currentTile.x -= 1; /* (x-1,y)*/
		comfortableExit(maze,currentTile,path,minWeight);
...
Path safestExit(TILES maze[100][100], Coord currentTile, Path path, int *minWeight){
	...
	if(maze[currentTile.x][currentTile.y].type == BORDER){
		...
		currentTile.x -= 1; /* (x-1,y)*/
		safestExit(maze,currentTile,path,minWeight);
