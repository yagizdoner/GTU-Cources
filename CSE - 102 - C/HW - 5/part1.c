...
int openCell(cell grid[][GRIDSIZE], int x, int y){
	...
	if( isLocationLegal(x,y) ){
		if( isCellEmpty(grid, x, y) ){
			grid[x][y] = empty ;
			if( isLocationLegal(x-1,y-1) && isCellEmpty(grid, x-1, y-1) && grid[x-1][y-1] != flaggedMined && grid[x-1][y-1] != flaggedEmpty ){
				grid[x-1][y-1] = empty ;
			}
			if( isLocationLegal(x-1,y) && isCellEmpty(grid, x-1, y) && grid[x-1][y] != flaggedMined && grid[x-1][y] != flaggedEmpty ){
				grid[x-1][y] = empty ;
			}
			if( isLocationLegal(x-1,y+1) && isCellEmpty(grid, x-1, y+1) && grid[x-1][y+1] != flaggedMined && grid[x-1][y+1] != flaggedEmpty ){
				grid[x-1][y+1] = empty ;                                                    			
			}
			if( isLocationLegal(x,y-1) && isCellEmpty(grid, x, y-1) && grid[x][y-1] != flaggedMined && grid[x][y-1] != flaggedEmpty ){
				grid[x][y-1] = empty ;                                                  			
			}
			if( isLocationLegal(x,y+1) && isCellEmpty(grid, x, y+1) && grid[x][y+1] != flaggedMined && grid[x][y+1] != flaggedEmpty ){
				grid[x][y+1] = empty ;                                                  			
