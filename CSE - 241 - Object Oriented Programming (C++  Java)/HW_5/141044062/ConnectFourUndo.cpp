...
namespace CellSpace{
	ConnectFourUndo::ConnectFourUndo(){
		undo = 1; // It can call the undo function.
		playGame();
		if(style == 'P')
			vsUsers();
		else if(style == 'C')
	...
	void ConnectFourUndo::vsComputer(){
		turn = 1;
		style='C';   
	...
	bool ConnectFourUndo::horizonalCheck(){
		int r,c, k;
	...
	bool ConnectFourUndo::verticalCheck(){
		int r,c, k;
		// r is row, c is column, k is a counter.
		whichCell letter;
		for(c=0 ;c<getColumn(); ++c){
	...
	bool ConnectFourUndo::totalCheck(){
