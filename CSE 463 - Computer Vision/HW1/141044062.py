...
_url="http://192.168.0.26:8080/shot.jpg" # Demo da Telefon IP kontrolünü sağla !!!
...
def main():        
    ...
    while(True):
        imgResp=url.urlopen(_url)
        imgNp=np.array(bytearray(imgResp.read()),dtype=np.uint8)
        frame=cv2.imdecode(imgNp,-1)
        cv2.imshow('PhoneCamera',frame)
             
        if cv2.waitKey(1) & 0xFF == ord('c'):
            img, testNp  = houghOperation(frame)
            res = perspectiveOperation(frame,testNp,modelNp,model)            
